...
Status checkBrackets(SeqStack* s, const char* expr) {
    for (int i = 0; expr[i] != '\0'; i++) {
        if (expr[i] == '(' || expr[i] == '{' || expr[i] == '[') {
            if (pushStack(s, expr[i]) == ERROR) {
                return ERROR;
            }
        } else if (expr[i] == ')' || expr[i] == '}' || expr[i] == ']') {
            DataType top;
            if (isStackEmpty(s) || popStack(s, &top) == ERROR) {
                return ERROR;
            }
            if ((expr[i] == ')' && top != '(') ||
                (expr[i] == '}' && top != '{') ||
...
Status evaluatePostfix(SeqStack* s, char exp[], int* result) {
    for (int i = 0; exp[i] != '\0'; i++) {
        if ('0' <= exp[i] && exp[i] <= '9') {
            if (pushStack(s, exp[i] - '0') == ERROR) {
                return ERROR;
            }
        } else {
            DataType b, a;
            if (popStack(s, &b) == ERROR || popStack(s, &a) == ERROR) {
                return ERROR;
            }
            DataType c;
            switch (exp[i]) {
