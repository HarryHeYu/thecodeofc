...
void Convert(long int n, int base){
    ...
    while(n != 0){
        long int remainder = n % base;//计算当前n除以基数base的余数
        n /= base;//更新n为商
        if(remainder < 0){
            remainder += (-base);
            n += 1;//调整余数和商，使其都为正，并且结果仍正确
            //n = base*n + remainder = base*(n+1) + (remainder-base)
        }
        if(remainder < 10){
...
例如当 $R=7$ 时,所需用到的数码是 $0,1,2,3,4,5,6$，这与其是 $R$ 或 $-R$ 无关。如果作为基数的数绝对值超过 $10$，则为了表示这些数码
，通常使用英文字母来表示那些大于 $9$ 的数码。例如对 $16$ 进制数来说,用 $A$ 表示 $10$，用 $B$ 表示 $11$，用 $C$ 表示 $12$，以此类推。
