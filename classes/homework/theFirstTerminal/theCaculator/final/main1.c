...
Status InfixToPostfix(char* infix, char* postfix) {
    ...
    while (infix[i] != '\0') {
        if (isdigit(infix[i]) || (infix[i] == '-' && (i == 0 || strchr("+-*/^(", infix[i - 1])))) {
            ...
            char c = infix[i];
            status = pushStack(&stack, &c);
            if (status != OK) {
                debugPrint("Error pushing '(': %s\n", infix);
                return status;
            }
            i++;
        } else if (infix[i] == ')') {
            ...
            while (!isStackEmpty(&stack) && (*(elem = (char*)peekStack(&stack)) != '(')) {
                ...
                if (status != OK) {
                    debugPrint("Error popping operator: %s\n", infix);
                    return status;
                }
                postfix[j++] = *op;
                postfix[j++] = ' ';
            ...
            if (!isStackEmpty(&stack)) {
                ...
                debugPrint("Mismatched parentheses in expression: %s\n", infix);
                return ERROR;
            }
            i++;
            ...
            status = pushStack(&stack, &c);
            if (status != OK) {
                debugPrint("Error pushing operator: %s\n", infix);
                return status;
            }
            i++;
        } else {
            debugPrint("Invalid character in expression: %s\n", infix);
            return ERROR;
        }
    }
    ...
    while (!isStackEmpty(&stack)) {
        char* op = (char*)popStack(&stack, &status);
        if (status != OK) {
            debugPrint("Error popping operator at end: %s\n", infix);
            return status;
        }
        if (*op == '(' || *op == ')') {
            free(op);
    ...
    while (postfix[i] != '\0') {
        if (isdigit(postfix[i]) || (postfix[i] == '-' && isdigit(postfix[i + 1]))) {
            ...
            *status = pushStack(&stack, &value);
            if (*status != OK) {
                debugPrint("Error pushing number: %s\n", number);
                return 0.0;
            }
        } else if (strchr("+-*/^", postfix[i]) != NULL) {
            double b = *(double*)popStack(&stack, status);
            if (*status != OK) {
                debugPrint("Error popping operand for operator %c\n", postfix[i]);
                return 0.0;
            }
            ...
            switch (postfix[i]) {
                ...
                default: 
                    *status = ERROR;
                    debugPrint("Invalid operator %c\n", postfix[i]);
                    return 0.0;
            }
            *status = pushStack(&stack, &result);
            if (*status != OK) {
                debugPrint("Error pushing result: %lf\n", result);
                return 0.0;
            }
            i++;
            ...
            if (*status != OK) {
                debugPrint("Error popping operand for function %c\n", postfix[i]);
                return 0.0;
            }
            result = applyFunction(postfix[i], value, status);
            if (*status != OK) {
                debugPrint("Error applying function %c\n", postfix[i]);
                return 0.0;
            }
            *status = pushStack(&stack, &result);
            if (*status != OK) {
                debugPrint("Error pushing result of function %c\n", postfix[i]);
                return 0.0;
            }
            i++;
        } else {
