...
MathFunction trigonometric_functions[] = {sin, cos, tan, asin, acos, atan, NULL};
...
double applyFunction(double arg, MathFunction func) {
    for (int i = 0; trigonometric_functions[i] != NULL; i++) {
        if (func == trigonometric_functions[i]) {
        ...
        if (isdigit(*p) || *p == '.') {
            ...
            *op = *p;
            ...
            while (!isStackEmpty(&operatorStack) && precedence(*((char*)operatorStack.items[operatorStack.top])) >= precedence(op) &&
                   (isRightAssociative(op) ? precedence(*((char*)operatorStack.items[operatorStack.top])) > precedence(op) :
                    precedence(*((char*)operatorStack.items[operatorStack.top])) >= precedence(op))) {
                if (isStackEmpty(&valueStack)) {
                    destroyStack(&valueStack);
                    destroyStack(&operatorStack);
                    return ERR_STACK_UNDERFLOW;
        ...
        if (errorCode != ERR_NONE) {
            switch (errorCode) {
                ...
                case ERR_STACK_OVERFLOW:
                    fprintf(outputFile, "Error: Stack overflow.\n");
                    break;
                default:
