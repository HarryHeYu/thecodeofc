...
ErrorCode expandStack(Stack *s) {
    ...
    void** newData = (void**)realloc(s->data, s->capacity * sizeof(void*));
    if (!newData) {
        return ERR_MEMORY_ALLOCATION;
    }
    s->data = newData;
    debugPrint("Expanded stack to new capacity: %d\n", s->capacity);
    return ERR_NONE;
}

ErrorCode pushStack(Stack *s, void* item) {
    if (s->top >= s->capacity - 1) {
        ErrorCode err = expandStack(s);
        if (err != ERR_NONE) {
            return err;
        }
    }
...
int main() {
    ...
    printf("Enter an expression (e.g., \"sin(pi/2) + sqrt(4)\"): ");
