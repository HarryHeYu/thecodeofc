//
// Created by He_Yu_Hao on 2025/4/29.
//
#include <stdio.h>
#include<stdlib.h>
#include <time.h>
...
void TraverseInorder_Nonrecursive(BinaryTree pRoot, void(*visit)(Elem*)){
    BinaryNode *pNode;
...
void Display(BinaryTree pRoot){
    ...
    left = 0;
    right = m - 1;
    LayOut(pRoot,(int*)a,m,left,right);
    printf("\n");
    for(i = 0; i < h; i++){
        for(j = 0; j < m; j++){
...
void LayOut(BinaryTree pRoot, int* a, int m, int left, int right){
    ...
    if(pRoot == nullptr) return;
    level++;
    a[level * m + (left + right) / 2] = pRoot->elem;
